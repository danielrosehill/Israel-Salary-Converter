...
if rates is not None:
    ...
    with tab1:
        ...
        st.markdown("Use the slider to select the Israeli salary and it will be converted.")
        ...
        with col2:
            if nis_salary:
                ...
                # Format the Israeli salary with 'k'
                nis_salary_k = f"{nis_salary/1000:.0f}k" if nis_salary >= 1000 else f"{nis_salary}"
                
                st.markdown(f"<p style='font-size: 2.5em; font-weight: bold;'>Israel 🇮🇱 =  <span style='color: #117A65;'>{nis_salary_k}</span></p>", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        st.markdown("Use the slider to select your salary and currency.")
        ...
        with col2:
            if world_salary:
                ...
                # Format world salary with 'k'
                world_salary_k = f"{world_salary/1000:.0f}k" if world_salary >= 1000 else f"{world_salary}"

                st.markdown(f"<p style='font-size: 2.5em; font-weight: bold;'>{selected_currency} = <span style='color: #117A65;'>{world_salary_k}</span></p>", unsafe_allow_html=True)

                st.subheader("Converted Salary")
                if ils_salary:
                  st.metric(label="🇮🇱 NIS", value=f"{ils_salary:,.0f} ₪")
                else:
